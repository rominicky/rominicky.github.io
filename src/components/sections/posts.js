...
const Posts = () => {
    ...
    query {
      posts: allMarkdownRemark(
        filter: {
          ...
          frontmatter: { category: { eq: "academic" } }
        }
  ...
  const postInner = node => {
    const { title, journal, year, url } = node.frontmatter;
  ...
  return (
    <StyledPostsSection>
      <h2 ref={revealTitle}>Artículos académicos</h2>
        ...
        {prefersReducedMotion ? (
          <>
            {postsToShow.map(({ node }, i) => (
              <StyledPosts key={i}>{postInner(node)}</StyledPosts>
            ))}
          </>
        ) : (
